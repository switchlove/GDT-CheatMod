...
(function() {
    ...
    div.append('<div id="cheatmod_score" style="text-align:center;margin-left:50px;width: 450px">Selected Review Score: </div>');
    div.append('<div class="scoreSlider"></div>');
    div.append('<div id="setScoreEnabled" class="selectorButton whiteButton" onclick="UI.pickCheatClick(this)" style="margin-left:50px;width: 450px">Activate always have selected scores</div>');
	...
	var getSelecetedScoreComment = function(score) {
		switch (score) {
			case 1:
				return ["N/A not worth a statement.".localize(), "One of the worst!".localize(), "Makes you cry.".localize(), "Don't buy!".localize(), "Horrible.".localize(), "I still have nightmares!".localize(), "A disaster!".localize(), "Really bad.".localize()].pickRandom();
			case 2:
				return ["Utterly uninspiring.".localize(), "Not fun.".localize(), "Boring.".localize(), "Disappointing.".localize(), "Bin material.".localize(), "Bad.".localize(), "Abysmal.".localize()].pickRandom();
			case 3:
				return ["Disappointing.".localize(), "Waste of money.".localize(), "Waste of time.".localize(), "Not much fun.".localize(), "Pretty bad.".localize()].pickRandom();
			case 4:
				return ["Not bad. Not good.".localize(), "Meh!".localize(), "OK.".localize(), "Uninspiring.".localize()].pickRandom();
			case 5:
				return ["Falls a bit short.".localize(), "Fun at stages.".localize(), "Has its moments.".localize(), "Have seen better.".localize()].pickRandom();
			case 6:
				return ["Shows potential.".localize(), "Could have been better.".localize(), "Quirky but good.".localize(), "I like it.".localize()].pickRandom();
			case 7:
				return ["Good game.".localize(), "Enjoyable.".localize(), "Nice experience.".localize(), "Beautiful.".localize()].pickRandom();
			case 8:
				return ["Very good.".localize(), "Very enjoyable.".localize(), "Love it!".localize(), "Played it for days.".localize()].pickRandom();
			case 9:
				return ["Great!".localize(), "Almost perfect.".localize(), "One of the best.".localize(), "More please.".localize(), "Great game.".localize(), "Outstanding game.".localize(), "Can't wait for the sequel.".localize()].pickRandom();
			case 10:
    ...
    var setScores = function(e) {
        var sliderSetScore = Number($("#CheatContainer").find("#cheatmod_score")[0].innerText.split(': ')[1]);
		console.log(sliderSetScore);

        e.reviews[0].score = sliderSetScore;
		e.reviews[0].message = getSelecetedScoreComment(sliderSetScore);
        e.reviews[1].score = sliderSetScore;
		e.reviews[1].message = getSelecetedScoreComment(sliderSetScore);
        e.reviews[2].score = sliderSetScore;
		e.reviews[2].message = getSelecetedScoreComment(sliderSetScore);
        e.reviews[3].score = sliderSetScore;
		e.reviews[3].message = getSelecetedScoreComment(sliderSetScore);
        e.game.score = sliderSetScore;
    };
    ...
    var setScoreEnabled = function() {
        if (selectedScores) {
            GDT.off(GDT.eventKeys.gameplay.afterGameReview, setScores);
            var div = $("#CheatContainer");
            div.find("#setScoreEnabled").html("Activate always have selected scores");
            selectedScores = false;
        } else {
            GDT.on(GDT.eventKeys.gameplay.afterGameReview, setScores);
            var div = $("#CheatContainer");
            div.find("#setScoreEnabled").html("Deactivate always have selected scores");
            selectedScores = true;
        }
    }
